...
func TestBaseRepository_CRUD(t *testing.T) {
	...
	// Find
	users, err := r.Find(ctx, query.New().Where(userProps.Age.Gte(30)).Order(userProps.Age.Asc()))
	require.NoError(t, err)
	...
	// First
	alice, err := r.First(ctx, query.New().Where(userProps.UserName.Eq("Alice")))
	require.NoError(t, err)
...
func TestBaseRepository_NotFoundReturnsNilEntity(t *testing.T) {
	...
	got, err := r.Take(ctx, query.New().Where(userProps.UserName.Eq("nobody")))
...
func TestBaseRepository_UpdateAndUpdates(t *testing.T) {
	...
	rows, err := r.Update(ctx, query.New().Where(userProps.UserName.Eq("Bob")), userProps.Age, 31)
	require.NoError(t, err)
	assert.Equal(t, int64(1), rows)

	rows, err = r.Updates(ctx, query.New().Where(userProps.UserName.Eq("Bob")), map[query.Column]any{
		userProps.Status: 2,
		userProps.Email:  "bob2@example.com",
	})
	require.NoError(t, err)
	assert.Equal(t, int64(1), rows)

	bob, err := r.First(ctx, query.New().Where(userProps.UserName.Eq("Bob")))
	require.NoError(t, err)
...
func TestBaseRepository_CreateInBatches(t *testing.T) {
	...
	count, err := r.Count(ctx, query.New().Where(userProps.UserName.In([]string{"A", "B"})))
	require.NoError(t, err)
...
func TestBaseRepository_Delete(t *testing.T) {
	...
	rows, err := r.Delete(ctx, query.New().Where(userProps.UserName.Eq("Alice")))
	require.NoError(t, err)
	assert.Equal(t, int64(1), rows)
...
func TestBaseRepository_Pluck(t *testing.T) {
	...
	var names []string
	err := r.Pluck(ctx, query.New().Order(userProps.ID.Asc()), userProps.UserName, &names)
	require.NoError(t, err)
...
func TestBaseRepository_Save(t *testing.T) {
	...
	got, err := r.First(ctx, query.New().Where(userProps.Email.Eq("alice@example.com")))
	require.NoError(t, err)
...
func TestBaseRepository_FirstTakeLast_SuccessAndNotFound(t *testing.T) {
	...
	// Take success
	got, err := r.Take(ctx, query.New().Where(userProps.UserName.Eq("Bob")))
	require.NoError(t, err)
	require.NotNil(t, got)
	assert.Equal(t, "Bob", got.UserName)

	// First not found
	gotFirst, err := r.First(ctx, query.New().Where(userProps.UserName.Eq("nobody")))
	require.ErrorIs(t, err, gorm.ErrRecordNotFound)
	require.Nil(t, gotFirst)

	// Last not found
	gotLast, err := r.Last(ctx, query.New().Where(userProps.UserName.Eq("nobody")))
	require.ErrorIs(t, err, gorm.ErrRecordNotFound)
