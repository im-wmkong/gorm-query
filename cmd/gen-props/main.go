// Package main 提供了一个用于生成强类型查询属性的命令行工具 (gen-props)。
//
// 该工具通过解析 Go 源码中的 AST (抽象语法树)，为指定的 GORM 模型结构体
// 生成对应的属性字典和 Alias 方法。生成的代码配合 query 包使用，
// 可以在编译期拦截字段名拼写错误，并优雅地解决连表查询时的字段歧义问题。
//
// 推荐使用 //go:generate 指令在模型文件中触发调用。
package main
