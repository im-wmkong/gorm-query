...
func TestTypeSafeColumnUsage(t *testing.T) {
	...
	q := New().Where(userColumns.Email.Eq("alice@example.com"))
...
func TestPagination_Page(t *testing.T) {
	...
	q := New().Order(userColumns.CreatedAt.Desc()).Page(1, 2)
...
func TestQuery_StringHelpers(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Where(userColumns.UserName.HasPrefix("Al")))
	require.NoError(t, err)
	require.Len(t, users, 1)
	assert.Equal(t, "Alice", users[0].UserName)

	users, err = applyFind(t, db, New().Where(userColumns.UserName.HasSuffix("lie")))
	require.NoError(t, err)
	require.Len(t, users, 1)
	assert.Equal(t, "Charlie", users[0].UserName)

	users, err = applyFind(t, db, New().Where(userColumns.UserName.Contains("li")))
	require.NoError(t, err)
	require.Len(t, users, 2)
	assert.Equal(t, "Alice", users[0].UserName)
	assert.Equal(t, "Charlie", users[1].UserName)

	users, err = applyFind(t, db, New().Where(userColumns.UserName.NotContains("a")))
	require.NoError(t, err)
	require.Len(t, users, 1)
...
func TestQuery_NotLike(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Where(userColumns.UserName.NotLike("%a%")))
	require.NoError(t, err)
	require.Len(t, users, 1)
	assert.Equal(t, "Bob", users[0].UserName)
}
...
func TestQuery_Select_Omit(t *testing.T) {
	...
	qSelect := New().Select(userColumns.UserName).Where(userColumns.UserName.Eq("Bob"))
	...
	qOmit := New().Omit(userColumns.Email).Where(userColumns.UserName.Eq("Bob"))
...
func TestQuery_Distinct(t *testing.T) {
	...
	q := New().Distinct(userColumns.UserName).Select(userColumns.UserName).Order(userColumns.UserName)
	users, err := applyFind(t, db, q)
	require.NoError(t, err)
...
func TestQuery_Between_AndColumnComparison(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Where(userColumns.Age.Between(20, 30)))
	require.NoError(t, err)
	require.Len(t, users, 3)

	users, err = applyFind(t, db, New().Where(userColumns.Age.Between(userColumns.Age, userColumns.Age)))
	require.NoError(t, err)
	require.Len(t, users, 5)

	users, err = applyFind(t, db, New().Where(userColumns.UserName.Like(userColumns.UserName)))
	require.NoError(t, err)
	require.Len(t, users, 5)
}
...
func TestQuery_ComparisonOps(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Where(userColumns.Age.Gt(30)))
	require.NoError(t, err)
	require.Len(t, users, 2)

	users, err = applyFind(t, db, New().Where(userColumns.Age.Gte(30)))
	require.NoError(t, err)
	require.Len(t, users, 3)

	users, err = applyFind(t, db, New().Where(userColumns.Age.Lt(25)))
	require.NoError(t, err)
	require.Len(t, users, 1)
	assert.Equal(t, "David", users[0].UserName)

	users, err = applyFind(t, db, New().Where(userColumns.Age.Lte(25)))
	require.NoError(t, err)
	require.Len(t, users, 2)

	users, err = applyFind(t, db, New().Where(userColumns.Age.Lt(userColumns.Age)))
	require.NoError(t, err)
	require.Empty(t, users)

	users, err = applyFind(t, db, New().Where(userColumns.Age.Lte(userColumns.Age)))
	require.NoError(t, err)
	require.Len(t, users, 5)

	users, err = applyFind(t, db, New().Where(userColumns.Age.Gt(userColumns.Age)))
	require.NoError(t, err)
	require.Empty(t, users)

	users, err = applyFind(t, db, New().Where(userColumns.Age.Gte(userColumns.Age)))
	require.NoError(t, err)
	require.Len(t, users, 5)
}
...
func TestQuery_In_NotIn(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Where(userColumns.UserName.In(names)))
	require.NoError(t, err)
	require.Len(t, users, 2)

	users, err = applyFind(t, db, New().Where(userColumns.UserName.NotIn(names)))
	require.NoError(t, err)
	require.Len(t, users, 3)
}
...
func TestQuery_Null_NotNull_WithSoftDelete(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Where(userColumns.DeletedAt.IsNull()))
	require.NoError(t, err)
	require.Len(t, users, 5)

	alice, err := applyFirst(t, db, New().Where(userColumns.UserName.Eq("Alice")))
	require.NoError(t, err)
	_, err = applyDelete(t, db, New().Where(userColumns.ID.Eq(alice.ID)))
	require.NoError(t, err)

	users, err = applyFind(t, db, New().Unscoped().Where(userColumns.DeletedAt.IsNotNull()))
	require.NoError(t, err)
	require.Len(t, users, 1)
	assert.Equal(t, "Alice", users[0].UserName)
}
...
func TestQuery_Or_Not_Clone_EmptyNested(t *testing.T) {
	...
	qOr := New().Where(userColumns.UserName.Eq("Alice")).Or(userColumns.UserName.Eq("Bob"))
	users, err := applyFind(t, db, qOr)
	require.NoError(t, err)
	require.Len(t, users, 2)

	base := New().Where(userColumns.Status.Eq(1))
	derived := base.Clone().Where(userColumns.UserName.Eq("Alice"))
	...
	qEmpty := New().Where(userColumns.Status.Eq(1)).Or().Not()
	users, err = applyFind(t, db, qEmpty)
	require.NoError(t, err)
	require.Len(t, users, 5)

	qNot := New().Not(userColumns.UserName.Eq("Alice"))
...
func TestQuery_Group_Having(t *testing.T) {
	...
	qGroup := New().Select(userColumns.Age).Group(userColumns.Age).Having("count(*) > ?", 1)
...
func TestQuery_Limit_Offset_Unscoped_Order_Joins_Scope(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Limit(2).Order(userColumns.ID))
	require.NoError(t, err)
	require.Len(t, users, 2)
	assert.Equal(t, "Alice", users[0].UserName)
	assert.Equal(t, "Bob", users[1].UserName)

	users, err = applyFind(t, db, New().Limit(2).Offset(2).Order(userColumns.ID))
	require.NoError(t, err)
	require.Len(t, users, 2)
	assert.Equal(t, "Charlie", users[0].UserName)
	assert.Equal(t, "David", users[1].UserName)
	...
	users, err = applyFind(t, db, New().Order(userColumns.ID))
	require.NoError(t, err)
	assert.Equal(t, "Alice", users[0].UserName)

	users, err = applyFind(t, db, New().Order(userColumns.ID.Desc()))
	require.NoError(t, err)
	assert.Equal(t, "admin", users[0].UserName)
...
func TestColumn_Helpers_AndNeq(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Where(userColumns.UserName.Neq("Alice")))
	require.NoError(t, err)
	require.Len(t, users, 4)

	assert.Equal(t, Column("users.age"), userColumns.Age.Table("users"))
	assert.Equal(t, Column("DISTINCT age"), userColumns.Age.Distinct())
	assert.Equal(t, Column("SUM(age)"), userColumns.Age.Sum())
	assert.Equal(t, Column("COUNT(age)"), userColumns.Age.Count())
	assert.Equal(t, Column("AVG(age)"), userColumns.Age.Avg())
	assert.Equal(t, Column("MIN(age)"), userColumns.Age.Min())
}
...
func TestQuery_NotBetween(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Where(userColumns.Age.NotBetween(20, 30)))
	require.NoError(t, err)
	require.Len(t, users, 2)
	for _, u := range users {
...
func TestQuery_OrderHelpers_AscDesc(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Order(userColumns.Age.Asc()))
	require.NoError(t, err)
	require.NotEmpty(t, users)
	assert.Equal(t, 20, users[0].Age)
	assert.Equal(t, 40, users[len(users)-1].Age)

	users, err = applyFind(t, db, New().Order(userColumns.Age.Desc()))
	require.NoError(t, err)
	require.NotEmpty(t, users)
...
func TestQuery_SelectHelpers_AsAndAgg(t *testing.T) {
	...
	qAs := New().Select(userColumns.Email.As("user_name")).Where(userColumns.UserName.Eq("Alice"))
	...
	// Max(Age)
	qMax := db.Model(&user{})
	qMax = New().Select(userColumns.Age.Max().As("age")).Apply(qMax)
	var out user
...
func TestQuery_OmitAndOrder_InvalidTypeAreSafe(t *testing.T) {
	...
	qOmitInvalid := New().Omit(123).Where(userColumns.UserName.Eq("Alice"))
	...
	qOrderInvalid := New().Order(123).Where(userColumns.UserName.Eq("Alice"))
...
func TestQuery_InEmptyDoesNotError(t *testing.T) {
	...
	users, err := applyFind(t, db, New().Where(userColumns.UserName.In([]string{})))
	require.NoError(t, err)
